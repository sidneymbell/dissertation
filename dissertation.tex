...
\Title{Viral phylodynamics of host adaptation and antigenic evolution}
...
\section{A framework for investigating host adaptation}
...
Returning to our previous example, the contrasting selective pressures presented by the host environments of dengue and influenza viruses are evident in the corresponding viral phylogenies (Figure~\ref{dengue_vs_influenza}) \citep{volz2013viral,bedford2011strength}.
...
And yet, despite this relatively low transmission rate, HIV has been acquired from primates via at least twelve independent cross-species tranmissions (CSTs).
This is particularly remarkable when one considers the many inherent challenges to viral replication in an entirely foreign host evironment (collectively referred to as the `species barrier').

The immediate history of HIV is well-established.
...
However, HIV-1 and -2 represent only two of over 45 distinct immunodeficiency viruses known to infect primates.
Although we understand the most immediate origins that generated the global pandemic, we lack crucial ecological context around how often these viruses switch hosts and what enabels a lentiviral cross-species transmission to take hold in a population.
...
To do so, I curated a dataset of publicly available lentiviral sequences, isolated from 24 primate host species; identified breakpoints of interlineage recombination across the viral genome; and reconstructed the evolutionary history of each genomic segment.
...
Although the vast timescale limits direct causal inference across the phylogeny, these cross-species transmissions are more common among pairs of closely related host species, which is likely a result of ecological circumstance and biological differences in innate immune factors.
...
As illustrated in Chapter 2, phylodynamics is a effective tool for retrospective, descriptive analyses; however, these methods are also powerful tools for explanatory, prospective analyses.
In Chapters 3 and 4, I apply phylodynamics to understand how dengue virus that adapts to evade human immunity and spread through populations.
...
Because the serotypes are so clearly distinct, and because dengue evolves so slowly overall, it has long been assumed that all viruses within a serotype are antigenically uniform (\textit{i.e.}, they look about the same to the host immune system).
...
To do so, I adapt a statistical model (originally developed for analyzing influenza's rapid antigenic evolution) to characterize dengue's slower, subtler antigenic dynamics.
In this model, each branch in the dengue phylogeny is treated as essentially one `step' in the overall evolutionary history.
I then map antigenic change to underlying genetic divergence by quanitfying how much antigenic change has occurred along each branch of the phylogeny.
...
However, when compared to pathogens that antigenically evolve very rapidly (such as influenza), the breadth of dengue antigenic diversity is very modest.
...
To test this hypothesis, I analyze dengue population dynamics across Southeast Asia over a thirty year time period.
...
Furthermore, the existance of large, ancient antigenic divergence between serotypes and smaller, recent antigenic divergence between sub-serotype clades suggests that dengue antigenic evolution is gradual but ongoing.
...
\subsubsection*{Main and supplemental datasets}
...
\subsection*{Phylogenies and discrete trait analysis}
...
Statistical support for each transmission is summarized as a Bayes factor (BF), calculated by comparing the posterior and prior odds that a given rate is non-zero (\textit{i.e.}, that there has been any transmission between a given pair of hosts) \citep{lemey2009bayesian}.
...
Collapsing putatively antigenically uniform clades, we observe at least 12 distinct antigenic phenotypes of DENV (Figure~\ref{antigenic_tree}), rejecting the null hypothesis that all dengue viruses within a serotype are antigenically uniform.
...
\subsection{Titer model strengths and limitations}
...
Many phylodynamic studies, such as the work presented in Chapter 2, are broad and descriptive in nature.
...
\section{Current methodological limitations and opportunities}
...
% Moving forward, consistent surveillance also stands to alleviate the uncertainty introduced by largely unsampled historical periods.
...
\section{Towards actionable inferences}
...
As the field progresses, phylodynamic inference is also poised to become a vital tool for public health and infectious disease epidemiology.
Phylodynamic studies are more actionable when they are prospective and specific.
In particular, dengue population dynamics has several open questions that would benefit from further study.

\subsection{Improving forecast accuracy}
...
I expect that error is introduced by two primary sources.

First, although dengue is a seasonal pathogen, all timepoints are treated equally in the current model.
...
Secondly, the current model formulation assumes that the entire region of Southeast Asia is a homogeneously mixed, closed population with no viral importation.
...
In order for this to be effective, however, we must also make our predictions more precise.

\subsection{Improving forecast specificity}
...
Thus, in order to provide actionable forecasts, we must also make these inferences in a timely manner, and on a finer geographic scale.
...
This data would enable far more geographically specific (and precise, as described above) predictions of which clades will dominate an upcoming epidemic season.
Over time, this would also provide a more spatially precise estimate of standing population immunity.
Given the antigenic distance between standing population immunity and expected dominant clades, we could feasibly estimate the proportion of infections that are expected to be severe in an upcoming epidemic season.

\subsection{Improving antigenic characterizations}
However, to forecast epidemic severity accurately, we also need to more precisely quantify the breadth and magnitude of dengue antigenic variation.
As extensively discussed in Chapter 3, the 12 antigenic phenotypes identified here are likely a significant underestimate of dengue antigenic diversity.
Generating larger datasets of standardized, pairwise titer measurements would enable a more comprehensive view of dengue antigenic diversity.
In conjunction with the above advancements, greater antigenic characterization would enable more accurate risk assessments for upcoming dengue seasons, given a precise estimation of local population immunity and incoming clades.

Throrough antigenic characterization would also enable two additional, major advances.
...
Additionally, a more nuanced understanding of dengue antigenic diversity would greatly facilitate vaccine strain selection.
Katzelnick \textit{et al.} quantitatively define the relationship between human titers and the probability of protection from severe secondary infection \citep{katzelnick2016neutralizing}.
...
\section{Conclusions}
% order of ideas; flesh out tomorrow
Phylodynamics is powerful.
...
\begin{centering}
  \begin{table}[ht!]
    \caption[Optimized parameter values for fitness model]{\textbf{Optimized parameter values for fitness model} }
    \resizebox{\textwidth}{!}{
...
\centering
	...
	\caption[Tree of dengue viruses in titer dataset]{\textbf{Tree of dengue viruses in titer dataset }
  Maximum likelihood phylogeny of all dengue viruses that were included in the titer dataset.
	}
