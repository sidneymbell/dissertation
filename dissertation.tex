...
This datas\textit{et al}so contains many left-censored titer values, where I know two viruses are at least $T$ titer units apart, but do not know exactly how far apart.
...
As discussed in Neher \textit{et al} and Lee \textit{et al}, I estimate empirical clade frequencies from 1970 to present based on observed relative abundance of each clade in the `slice' of the phylogeny corresponding to each quarterly timepoint \citep{lee2018deep,neher2016prediction}.
...
For each dataset, I jointly fit these parameters to maximize $\Delta SSE$ (Table ??).
% \end{fold}
...
\begin{figure}[h!]
  \begin{centering}
    \includegraphics[width=.8\linewidth]{./png/siv_pruneddata_cst.png}
  	\caption{\textbf{Network of inferred CSTs of primate lentiviruses ("main" dataset, after removing taxa shorter than 100 bases from each segment) }
The phylogeny of the host species' mitochondrial genomes forms the outer circle.
...
\begin{figure}[h!]
  \begin{centering}
    \includegraphics[width=\linewidth]{./png/siv_pruneddata_trees.png}
  	\caption{\textbf{Maximum clade credibility trees for each of the 12 GARD-identified genomic segments of the lentiviral genome ("main" dataset, after removing taxa shorter than 100 bases from each segment) }
Tips are color coded by known host state; branches and internal nodes are color coded by inferred host state, with color saturation indicating the confidence of these assignments.
Monophyletic clades of viruses from the same lineage are collapsed, with the triangle width proportional to the number of represented sequences.
        }
